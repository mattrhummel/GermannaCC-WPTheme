...
<ul class='bxslider1'>
	...
	<a href="<?php echo $slide_link ?>" aria-label="<?php the_title(); ?>"><?php endif; ?><?php $my_meta = get_post_meta( $post->ID, 'header_link', true ); ?><?php if ( $my_meta && '' != $my_meta ) : { ?><?php if ( has_post_thumbnail() ) : the_post_thumbnail('home-slider') ; /*loads the slide image*/ { if ( has_excerpt() ) {  // This post has excerpt.
		//the caption buttons
